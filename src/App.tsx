...
function DarkHeroCard({ debate, onClick, isMobile }: { debate: Debate; onClick: () => void; isMobile: boolean }) {
  ...
  return (
    <div onClick={onClick} style={{
      display: 'grid',
      gridTemplateColumns: isMobile ? '1fr 128px' : `1fr ${portraitColWidth}px`,
      minHeight: isMobile ? 260 : 230, cursor: 'pointer',
      ...
      {/* Left text */}
      <div style={{ padding: isMobile ? '20px 16px' : '40px 32px', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
        ...
        <div style={{ fontFamily: 'var(--font-display)', fontSize: isMobile ? 18 : 30, fontWeight: 700, color: '#fff', letterSpacing: '-0.01em', lineHeight: 1.2, marginBottom: 12 }}>
          {debate.title}
        </div>
      ...
      <div style={{
        ...
        gap: isMobile ? 4 : 8, padding: isMobile ? 4 : 8,
        ...
        {participants.slice(0, isMobile ? 2 : 7).map((p, i) => {
          ...
          return (
            <div key={p.person.id || i} style={{
              ...
              {!isMobile && <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '40%', background: `linear-gradient(to top, ${glow}ee 0%, transparent 100%)` }} />}
              <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '4px 5px 5px', display: 'flex', flexDirection: 'row', alignItems: isMobile ? 'flex-end' : 'center', justifyContent: isMobile ? 'flex-end' : 'flex-start', gap: 3 }}>
                ...
                <PartyBadge party={p.person.party} size={isMobile ? 14 : 16} radius={4} />
              </div>
            </div>
          )
        })}
      ...
      {debates.map((debate) => {
        ...
        return (
          <div key={debate.id} onClick={() => onSelect(debate.id)} style={{
            ...
            {/* Portraits */}
            <div style={{ display: 'flex', gap: 5, paddingBottom: 14, flexWrap: 'wrap' }}>
              {participants.slice(0, 4).map((p, pi) => {
                const sz = 38
                return (
                  <div key={p.person.id || pi} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
                    <div style={{ width: sz, height: sz, borderRadius: 9, overflow: 'hidden', background: '#1e1a40', position: 'relative', boxShadow: '0 2px 6px rgba(0,0,0,0.3)' }}>
                      <img src={p.person.photoUrl} alt={p.person.name} loading="lazy"
                        onError={e => { (e.target as HTMLImageElement).style.display = 'none' }}
                        style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'top center' }} />
                    </div>
                    <PartyBadge party={p.person.party} size={14} />
                  </div>
                )
              })}
              {participants.length > 4 && (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
                  <div style={{ width: 38, height: 38, borderRadius: 9, background: 'rgba(155,125,255,0.15)', border: '1px solid rgba(155,125,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: '#9b7dff' }}>
                    +{participants.length - 4}
                  </div>
                  <div style={{ height: 14 }} />
                </div>
  ...
  return (
    <div onClick={onClick} style={{
      ...
      <div style={{ padding: isMobile ? '20px 16px' : '40px 32px', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
        ...
        <div style={{ fontFamily: 'var(--font-display)', fontSize: isMobile ? 18 : 30, fontWeight: 700, color: '#111', letterSpacing: '-0.01em', lineHeight: 1.2, marginBottom: 12 }}>
          {debate.title}
        </div>
          ...
          <div key={debate.id} onClick={() => onSelect(debate.id)} style={{
            ...
            <div style={{ display: 'flex', gap: 5, paddingBottom: 14, flexWrap: 'wrap' }}>
              {participants.slice(0, 4).map((p, pi) => {
                ...
                return (
                  <div key={p.person.id || pi} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
                    <div style={{ width: sz, height: sz, borderRadius: 9, overflow: 'hidden', background: '#ddd8d0', position: 'relative', boxShadow: '0 2px 6px rgba(0,0,0,0.08)' }}>
                      <img src={p.person.photoUrl} alt={p.person.name} loading="lazy"
                        onError={e => { (e.target as HTMLImageElement).style.display = 'none' }}
                        style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'top center' }} />
                    </div>
              ...
              {participants.length > 4 && (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
                  <div style={{ width: 38, height: 38, borderRadius: 9, background: 'rgba(102,68,204,0.1)', border: '1px solid rgba(102,68,204,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: '#6644cc' }}>
                    +{participants.length - 4}
                  </div>
                  <div style={{ height: 14 }} />
                </div>
